# Build app/cam_capture_sd — QVGA JPEG to microSD (Phase 2 step 2.4)
# Usage: .\scripts\build-cam-capture-sd.ps1 [-Flash] [-Monitor] [-Port COM16] [-Pristine]
...
$AppDir = Join-Path $RepoRoot "app\cam_capture_sd"
$Board = "xiao_esp32s3/esp32s3/procpu/sense"
$ExtraConf = (Join-Path $RepoRoot "config\cam-capture-sd-sense.conf") -replace '\\', '/'
...
if ($Flash) {
    ...
    Write-Host "Expect: boot capture Wrote /SD:/ZEPHR000.JPG, then press BOOT for more"
    Write-Host "When done: .\scripts\kill-serial-monitor.ps1 -Port $Port"
    if ($Monitor) {
        & (Join-Path $RepoRoot "scripts\capture-serial.ps1") -Port $Port `
            -OutFile "cam_jpeg_capture.txt" -Seconds 45
    }
}
